"use client";

/**
 * Worker leaderboard with two lenses LightChain's own explorer doesn't offer:
 *   - "Reliable": ranks by completion rate then p95 latency (who is dependable).
 *   - "At risk":  ranks by quantified exposure the directory never shows -
 *     stake slash exposure from currently-stuck jobs, suspension proximity
 *     (lifetime timeouts vs the AIConfig threshold), and below-stake-floor.
 * Reads /api/analytics?risk=1 (one network sample + 2 joins, no per-worker fan-out).
 */
import { useEffect, useState } from "react";
import Link from "next/link";
import { ShieldCheck, ShieldAlert, RefreshCw } from "lucide-react";
import { Card } from "@/components/ui/card";
import { fmt, shortAddr, cn } from "@/lib/utils";
import { useNetwork } from "@/lib/network-context";

interface RiskWorker {
  address: string;
  total: number;
  completionRate: number | null;
  p95: number | null;
  earnings: number;
  stuck: number;
  stakeLcai?: number | null;
  lifetimeTimeouts?: number | null;
  belowFloor?: boolean;
  suspensionRisk?: boolean;
  slashExposureLcai?: number;
}

type Mode = "reliable" | "risk";

function isAtRisk(w: RiskWorker): boolean {
  return !!w.belowFloor || !!w.suspensionRisk || (w.slashExposureLcai ?? 0) > 0;
}

function RiskBadges({ w }: { w: RiskWorker }) {
  const badges: { label: string; tone: string }[] = [];
  if ((w.slashExposureLcai ?? 0) > 0) badges.push({ label: `~${fmt(w.slashExposureLcai ?? 0, 0)} LCAI at risk`, tone: "text-warning border-warning/30 bg-warning/10" });
  if (w.suspensionRisk) badges.push({ label: "suspension risk", tone: "text-danger border-danger/30 bg-danger/10" });
  if (w.belowFloor) badges.push({ label: "below floor", tone: "text-danger border-danger/30 bg-danger/10" });
  if (badges.length === 0) return <span className="text-content-soft">-</span>;
  return (
    <span className="flex flex-wrap justify-end gap-1">
      {badges.map((b) => (
        <span key={b.label} className={cn("rounded-full border px-1.5 py-0.5 text-[10px] font-medium", b.tone)}>
          {b.label}
        </span>
      ))}
    </span>
  );
}

export function ReliabilityLeaderboard() {
  const { network } = useNetwork();
  const [rows, setRows] = useState<RiskWorker[] | null>(null);
  const [err, setErr] = useState(false);
  const [mode, setMode] = useState<Mode>("reliable");

  useEffect(() => {
    let on = true;
    setRows(null);
    setErr(false);
    const load = () =>
      fetch(`/api/analytics?net=${network}&risk=1`)
        .then((r) => r.json())
        .then((j) => on && (j.ok ? setRows(j.workers ?? []) : setErr(true)))
        .catch(() => on && setErr(true));
    load();
    const t = setInterval(load, 30_000);
    return () => {
      on = false;
      clearInterval(t);
    };
  }, [network]);

  const reliable = (rows ?? [])
    .filter((w) => w.total >= 3 && w.completionRate != null)
    .sort((a, b) => (b.completionRate ?? 0) - (a.completionRate ?? 0) || (a.p95 ?? 1e9) - (b.p95 ?? 1e9))
    .slice(0, 15);

  const atRisk = (rows ?? [])
    .filter(isAtRisk)
    .sort(
      (a, b) =>
        (b.slashExposureLcai ?? 0) - (a.slashExposureLcai ?? 0) ||
        Number(b.suspensionRisk) - Number(a.suspensionRisk) ||
        Number(b.belowFloor) - Number(a.belowFloor),
    )
    .slice(0, 15);

  const ranked = mode === "reliable" ? reliable : atRisk;
  const rateTone = (r: number | null) => (r == null ? "text-content-soft" : r >= 0.95 ? "text-success" : r >= 0.85 ? "text-warning" : "text-danger");

  return (
    <Card className="overflow-hidden">
      <div className="flex flex-wrap items-center gap-2 border-b border-bdr-soft px-5 py-4">
        {mode === "reliable" ? <ShieldCheck className="size-4 text-success" /> : <ShieldAlert className="size-4 text-warning" />}
        <h2 className="text-sm font-semibold text-content-primary">Workers</h2>
        <div className="flex rounded-lg border border-bdr-soft p-0.5 text-xs">
          {(["reliable", "risk"] as Mode[]).map((m) => (
            <button
              key={m}
              type="button"
              onClick={() => setMode(m)}
              className={cn(
                "rounded-md px-2 py-0.5 font-medium transition-colors",
                mode === m ? "bg-primary/10 text-content-primary" : "text-content-soft hover:text-content-primary",
              )}
            >
              {m === "reliable" ? "Most reliable" : "At risk"}
            </button>
          ))}
        </div>
        <span className="ml-auto inline-flex items-center gap-1.5 text-xs text-content-soft">
          <RefreshCw className="size-3" /> 30s
        </span>
      </div>
      {err && !rows && <p className="px-5 py-8 text-center text-sm text-content-soft">Analytics unavailable right now.</p>}
      <div className="divide-y divide-bdr-light">
        <div className="flex items-center gap-3 px-5 py-2 text-[11px] font-medium uppercase tracking-wide text-content-soft">
          <span className="w-6 text-right">#</span>
          <span className="flex-1">Worker</span>
          {mode === "reliable" ? (
            <>
              <span className="w-20 text-right">Complete</span>
              <span className="hidden w-16 text-right sm:block">p95</span>
              <span className="w-24 text-right">Earned</span>
            </>
          ) : (
            <span className="flex-1 text-right">Exposure</span>
          )}
        </div>
        {(ranked.length ? ranked : rows ? [] : Array.from({ length: 6 }, () => null)).map((w, i) => (
          <div key={w?.address ?? i} className="flex items-center gap-3 px-5 py-3 text-sm">
            <span className="w-6 text-right font-mono text-content-soft">{i + 1}</span>
            <Link href={`/worker/${w?.address ?? ""}`} className="flex-1 truncate font-mono text-content-primary hover:text-primary">
              {w ? shortAddr(w.address) : "-"}
            </Link>
            {mode === "reliable" ? (
              <>
                <span className={cn("w-20 text-right font-semibold", rateTone(w?.completionRate ?? null))}>
                  {w && w.completionRate != null ? `${Math.round(w.completionRate * 100)}%` : "-"}
                </span>
                <span className="hidden w-16 text-right text-content-soft sm:block">{w?.p95 != null ? `${w.p95}s` : "-"}</span>
                <span className="w-24 text-right font-medium text-success">{w ? `${fmt(w.earnings, 2)}` : "-"}</span>
              </>
            ) : (
              <span className="flex-1 text-right">{w ? <RiskBadges w={w} /> : "-"}</span>
            )}
          </div>
        ))}
        {rows && ranked.length === 0 && !err && (
          <p className="px-5 py-8 text-center text-sm text-content-soft">
            {mode === "reliable" ? "Not enough recent jobs to rank reliability yet." : "No workers at risk in the recent sample - the network is healthy."}
          </p>
        )}
      </div>
    </Card>
  );
}
