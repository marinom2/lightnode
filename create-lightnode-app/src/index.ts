...
import { filesFor, type ProjectConfig } from "./templates.js";

const TEMPLATES: ProjectConfig["template"][] = ["node", "nextjs-api", "hono"];
const NETWORKS: ProjectConfig["network"][] = ["testnet", "mainnet"];
...
async function main() {
  ...
  // 2. Template.
  let template = (flag("template") as ProjectConfig["template"] | undefined) ?? null;
  if (!template) {
    const choice = await ask(
      `Template (${TEMPLATES.join(" | ")})`,
      "node",
    );
    if (!TEMPLATES.includes(choice as ProjectConfig["template"])) {
      console.error(`✗ unknown template "${choice}". Use one of: ${TEMPLATES.join(", ")}`);
      process.exit(1);
    }
    template = choice as ProjectConfig["template"];
  }

  // 3. Network.
  let network = (flag("network") as ProjectConfig["network"] | undefined) ?? null;
  if (!network) {
    const choice = await ask(`Default network (${NETWORKS.join(" | ")})`, "testnet");
    if (!NETWORKS.includes(choice as ProjectConfig["network"])) {
      console.error(`✗ unknown network "${choice}". Use testnet or mainnet`);
      process.exit(1);
    }
    network = choice as ProjectConfig["network"];
  }

  const cfg: ProjectConfig = { projectName, template, network };
