...
const CHANGELOG = [
  { v: "0.14.0", date: "June 2026", line: "Reliability: GatewayClient now auto-retries 429 (rate limit) and 5xx with exponential backoff, honoring Retry-After (configurable via { retry: { maxRetries, baseDelayMs } }); GatewayHttpError gains isRateLimited / isAuthError / isServerError + retryAfterMs. LightNode accepts { cacheTtlMs } to TTL-memoize the network-wide reads (getModels / getNetworkStats / getModelStats / getNetworkAnalytics / getWorkerStats) with a clearCache() escape hatch - so a polling dashboard stops re-hitting the indexer every render." },
